                        ...
                        <div class="flex items-start justify-between">
                            ...
                            <div class="flex space-x-4 flex-1">
                                <img src="{{ $item->buku->cover ? asset('storage/' . $item->buku->cover) : 'https://via.placeholder.com/80x120/4F46E5/FFFFFF?text=No+Cover' }}"
                                    alt="{{ $item->buku->judul }}" class="w-16 h-24 object-cover rounded-lg shadow-md">
                            ...
                            <div class="ml-4 flex flex-col items-end space-y-3">
                                ...
                                <div class="flex flex-wrap gap-2 justify-end">
                                    ...
                                    @if($item->status_peminjaman === 'dipinjam' && !$item->isEbook())
                                    ...
                                    @elseif($item->status_peminjaman === 'dipinjam' && $item->isEbook())
                                    ...
                                    @if($item->status_peminjaman === 'dikembalikan' && $item->denda > 0 && !$item->denda_lunas)
                                        <form action="{{ route('peminjaman.konfirmasi-denda', $item->id) }}" method="POST"
                                            class="inline">
                                            @csrf
                                            <button type="submit"
                                                class="px-4 py-2 bg-purple-500 text-white rounded-lg hover:bg-purple-600 transition text-sm font-medium">
                                                💰 Konfirmasi Denda
                                            </button>
                                        </form>
                                    @endif
