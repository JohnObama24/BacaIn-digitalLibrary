...
<div class="space-y-6">
    ...
    <div class="md:flex md:justify-between gap-5 items-center">
        ...
        <a
            ...
            class="md:w-96 px-6 py-4 bg-primary-blue text-white rounded-lg hover:bg-primary-blue/80 transition flex items-center shadow-lg hover:shadow-xl"
        >
        ...
        <table class="w-full" id="booksTable">
            ...
            <tbody class="divide-y divide-gray-200">
                ...
                <tr class="hover:bg-gray-50 transition book-row">
                   ...
                   <td class="px-6 py-4 text-center">
                        @if ($user->role == 'admin')
                            <span class="inline-flex items-center px-3 py-1 text-xs font-medium bg-[#F3D1FF] text-[#9800BE] rounded-full">
                                {{ $user->role }}
                            </span>
                        @elseif ($user->role == 'user')
                            <span class="inline-flex items-center px-3 py-1 text-xs font-medium bg-[#CDEFFF] text-[#0016BE] rounded-full">
                                {{ $user->role }}
                            </span>
                        @else
                            <span class="inline-flex items-center px-3 py-1 text-xs font-medium bg-[#E0E0E0] text-[#515151] rounded-full">
                                {{ $user->role }}
                            </span>
                        @endif
                    </td>
                ...
                <td class="px-6 py-4 text-center">
                    @if ($user->status == 'Aktif')
                        <span class="inline-flex items-center px-3 py-1 text-xs font-medium bg-green-100 text-green-700 rounded-full">
                            Aktif
                        </span>
                    @else
                ...
                <tr>
                    <td colspan="9" class="px-6 py-12 text-center">
                        <div class="flex flex-col items-center justify-center">
                            ...
                            <button type="button" class="mt-4 text-indigo-600 hover:text-indigo-700 font-medium">
                                Tambah User Baru
                            </button>
                        </div>
                    </td>
