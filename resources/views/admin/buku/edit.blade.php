...
@section('subheader', 'Perbarui informasi buku')
        ...
        <form action="{{ route('buku.update', $buku->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="border-b pb-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="number" name="tahun_terbit" value="{{ old('tahun_terbit', $buku->tahun_terbit) }}" required min="1900" max="{{ date('Y') }}"
                               class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">
                    </div>
                    ...
                    <div>
                        ...
                        <input type="number" name="jumlah_halaman" value="{{ old('jumlah_halaman', $buku->jumlah_halaman) }}" required min="1"
                               class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">
                    </div>
                </div>
            </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select name="status_buku" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">
                            <option value="tersedia" {{ old('status_buku', $buku->status_buku) == 'tersedia' ? 'selected' : '' }}>Tersedia</option>
                            <option value="tidak tersedia" {{ old('status_buku', $buku->status_buku) == 'tidak tersedia' ? 'selected' : '' }}>Tidak Tersedia</option>
                        </select>
                    </div>
                </div>
            </div>
            ...
            <div class="border-b pb-6">
                ...
                <textarea name="deskripsi" rows="5" required
                          class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                          placeholder="Tulis sinopsis atau deskripsi buku...">{{ old('deskripsi', $buku->deskripsi) }}</textarea>
            </div>
            ...
            <div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        @if($buku->cover)
                            <div class="mb-3">
                                <img src="{{ asset('storage/'.$buku->cover) }}" alt="Current Cover" class="w-32 h-44 object-cover rounded-lg shadow-md">
                                <p class="text-xs text-gray-500 mt-1">Cover saat ini</p>
                            </div>
                        @endif
                        <input type="file" name="cover" accept="image/*"
                               class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">
                        <p class="mt-1 text-xs text-gray-500">Format: JPG, JPEG, PNG. Max: 2MB. Kosongkan jika tidak ingin mengubah.</p>
                    </div>
                        ...
                        @if($buku->isi_buku)
                            <div class="mb-3 flex items-center space-x-2">
                                ...
                                <div>
                                    ...
                                    <p class="text-xs text-gray-500">{{ basename($buku->isi_buku) }}</p>
                                </div>
                            </div>
                        @endif
                        <input type="file" name="isi_buku" accept=".pdf"
                               class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">
                        <p class="mt-1 text-xs text-gray-500">Format: PDF. Max: 20MB. Kosongkan jika tidak ingin mengubah.</p>
                    </div>
                </div>
            </div>
